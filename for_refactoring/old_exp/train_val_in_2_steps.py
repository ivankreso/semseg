...
def validate(checkpoint_path, dataset):
  tf.reset_default_graph()
  image, labels, weights, num_labels = reader.inputs(dataset)
  ...
  for i in trange(dataset.num_examples()):
    ...
    y = out_logits[0].argmax(2)
    CollectConfusionMatrix(y.reshape(-1), yt, conf_mat)
    #ComputeErrors(conf_mat, 'validation', class_info)
  ComputeErrors(conf_mat, 'validation', class_info)
  tf.reset_default_graph()


def train(checkpoint_path, dataset, epoch_num):
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  for epoch in range(FLAGS.num_epochs):
    checkpoint_path = train(checkpoint_path, train_data, epoch)
    validate(checkpoint_path, val_data)
