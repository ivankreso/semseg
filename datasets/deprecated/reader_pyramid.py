...
def _read_and_decode(filename_queue):
  ...
  features = tf.parse_single_example(
      ...
      features={
          ...
          'routing_data': tf.FixedLenFeature([], tf.string),
  ...
  routing_data = tf.decode_raw(features['routing_data'], tf.int32, name='decode_routing_data')
  ...
  embed_height = FLAGS.img_height // FLAGS.net_subsampling
  embed_width = FLAGS.img_width // FLAGS.net_subsampling
  num_scales = 3
  image = tf.reshape(image, shape=[FLAGS.img_height, FLAGS.img_width, FLAGS.img_depth])
  routing_data = tf.reshape(routing_data, shape=[embed_height, embed_width, num_scales])
  ...
  return image, routing_data, labels, weights, num_labels, img_name
...
def inputs(dataset, shuffle=True, num_epochs=None):
  ...
  with tf.name_scope('input'), tf.device('/cpu:0'):
    ...
    image, routing, labels, weights, num_labels, img_name = _read_and_decode(filename_queue)

    # Shuffle the examples and collect them into batch_size batches.
    # Run this in two threads to avoid being a bottleneck.
    image, routing, labels, weights, num_labels, img_name = tf.train.batch(
        [image, routing, labels, weights, num_labels, img_name], batch_size=FLAGS.batch_size,
        num_threads=2, capacity=64)

    return [image, routing], labels, weights, num_labels, img_name
