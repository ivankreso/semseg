...
tf.app.flags.DEFINE_string('save_dir',
    ...
    '{}x{}'.format(FLAGS.img_width, FLAGS.img_height) + '_jitter/', '')
...
def prepare_dataset_jitter():
  ...
  print('Preparing jitter for ' + name)
  root_dir = '/home/kivan/datasets/Cityscapes/crops/crops'
  save_dir = join(FLAGS.save_dir, name)
  ...
  for i in trange(len(img_list)):
    ...
    depth_dummy = np.zeros_like(gt_img, dtype=np.uint8)
    create_tfrecord(rgb, gt_img, gt_weights, depth_dummy,
