...
tf.app.flags.DEFINE_string('save_dir', '/home/kivan/datasets/Cityscapes/tensorflow/'
    + '{}x{}'.format(FLAGS.img_width, FLAGS.img_height) + '_multiscale/', '')
...
def prepare_dataset(name):
  ...
  #depth_hist = np.zeros(128, dtype=np.uint64)
  for city in cities:
    print(city)
    ...
    for i in trange(len(img_list)):
      ...
      #depth_sum += depth_img
      #for d in range(depth_hist.shape[0]):
      #  depth_hist[d] += (depth_img == d).sum()
      #depth_ratios = depth_hist / depth_hist.sum()
      #acc = 0
      #for i in range(depth_hist.shape[0]):
      #  acc += depth_ratios[i]
      #  #print(i, " -> ", acc)
      ...
      num_ranges = len(DEPTH_THR) - 1
      gt_shape = gt_ids.shape
      gt_imgs = np.zeros((num_ranges, gt_shape[0], gt_shape[1]), dtype=gt_ids.dtype)
      for i in range(num_ranges):
        mask = np.logical_not(np.logical_and(depth_img >= DEPTH_THR[i], depth_img < DEPTH_THR[i+1]))
        gt_img = gt_ids.copy()
        gt_img[mask] = -1
        #summm += (gt_img >= 0).sum())
        gt_imgs[i] = gt_img
      ...
      create_tfrecord(rgb, gt_imgs, gt_weights, depth_img, num_labels, img_prefix, save_dir)
