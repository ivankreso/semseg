...
def conv2d(inputs,
  ...
  with tf.variable_op_scope([inputs], scope, 'Conv', reuse=reuse):
    ...
    if dilation == None or dilation == 1:
      ...
      conv = tf.nn.atrous_conv2d(inputs, weights, rate=dilation, padding=padding)
...
def fc(inputs,
  ...
  with tf.variable_op_scope([inputs], scope, 'FC', reuse=reuse):
    ...
    TODO
    weights_initializer = tf.contrib.layers.variance_scaling_initializer(seed=FLAGS.seed)
    l2_regularizer = None
    if weight_decay and weight_decay > 0:
      l2_regularizer = losses.l2_regularizer(weight_decay)
    weights = variables.variable('weights',
