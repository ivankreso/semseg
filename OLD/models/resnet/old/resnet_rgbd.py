...
def _build(image, depth, is_training):
  ...
  with tf.variable_scope('head'):
    with arg_scope([layers.convolution2d],
      ...
      #l = tf.concat(3, [l, lp])
      l = pyramid_pooling(l, 'pyramid_pooling')
      depth = tf.image.resize_bilinear(depth, l.get_shape().as_list()[1:3],
                                      name='resize_depth')
      l = tf.concat(3, [l, depth])
      #l = layers.convolution2d(l, 1024, kernel_size=1, scope='conv1') # faster
      l = layers.convolution2d(l, 512, kernel_size=3, scope='conv1') # faster
      #l = layers.convolution2d(l, 512, kernel_size=5, rate=2, scope='conv2')
      #l = layers.convolution2d(l, 512, kernel_size=5, rate=8, scope='conv2')
...
def build(dataset, is_training, reuse=False):
  ...
  image, labels, weights, depth, img_names = reader.inputs(
  ...
  logits, logits2 = _build(image, depth, is_training)
  total_loss = loss(logits, logits2, labels, weights, is_training)
...
def loss(logits, logits2, labels, weights, is_training=True):
  # TODO
