...
def build(inputs, labels, weights, num_labels, is_training=True):
  ...
  with tf.contrib.framework.arg_scope([layers.convolution2d],
    ...
    with tf.contrib.framework.arg_scope([layers.convolution2d],
      ...
      vgg_out = convolve(net, 512, 3, 'conv5_3', VGG_INIT)
      ...
      for i, k in enumerate(pool_rates):
        #print(r, i)
        if k > 1:
          net = layers.max_pool2d(net, k, k, padding='SAME', scope='pool'+str(5+i))
        net = layers.convolution2d(net, 512, conv_kernels[i], scope='conv6_'+str(i+1)+'_1')
        net = layers.convolution2d(net, 512, 1, scope='conv6_'+str(i+1)+'_2')
        print(net)
        levels += [net]
        ...
        if i == 0:
          ...
          net = tf.image.resize_bilinear(net, [top_height, top_width],
                                         name='conv6_upsample_'+str(i+1))
          top_layer = tf.concat(3, [top_layer, net])
        # try concat

      net = layers.convolution2d(top_layer, 1024, 3, scope='conv7_1')
      net = layers.convolution2d(net, 512, 1, scope='conv7_2')
    ...
    net = layers.convolution2d(net, num_classes, 1, activation_fn=None, scope='scores')
    logits = tf.image.resize_bilinear(net, [FLAGS.img_height, FLAGS.img_width],
                                      name='resize_score')
    loss_val = loss(logits, labels, weights, num_labels, is_training)
  return logits, loss_val, [logits]
