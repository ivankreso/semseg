...
def _read_and_decode(filename_queue):
  ...
  labels = tf.to_float(tf.decode_raw(features['disparity'], tf.uint8, name='decode_depth'))
...
def inputs(dataset, shuffle=True, num_epochs=None):
  ...
  with tf.name_scope('input'), tf.device('/cpu:0'):
    ...
    image, labels, img_name = _read_and_decode(filename_queue)

    # Shuffle the examples and collect them into batch_size batches.
    # Run this in two threads to avoid being a bottleneck.
    image, labels, img_name = tf.train.batch([image, labels, img_name],
        batch_size=FLAGS.batch_size, num_threads=2, capacity=64)
