...
def build_refinement_module(top_layer, skip_data):
  ...
  if data_format == 'NCHW':
    top_layer = tf.transpose(top_layer, perm=[0,2,3,1])
...
def jitter(image, labels, weights, depth):
  with tf.name_scope('jitter'), tf.device('/cpu:0'):
    ...
    for i in range(FLAGS.batch_size):
      out_img.append(tf.cond(random_flip_tf[i], lambda: tf.image.flip_left_right(image_split[i]),
                             lambda: image_split[i]))
      out_depth.append(tf.cond(random_flip_tf[i], lambda: tf.image.flip_left_right(depth_split[i]),
...
def minimize(loss, global_step, num_batches):
  ...
  lr_fine = tf.train.exponential_decay(base_lr / 5, global_step, decay_steps,
