  ...
  with tf.contrib.framework.arg_scope([layers.convolution2d],
    ...
    with tf.contrib.framework.arg_scope([layers.convolution2d],
      ...
      for i, level in enumerate(vgg_levels):
        net = layers.convolution2d(level, 512, conv_kernels[i], scope='conv6_'+str(i+1)+'_1')
        net = layers.convolution2d(net, 512, 3, scope='conv6_'+str(i+1)+'_2')
        net = layers.convolution2d(net, 512, 3, scope='conv6_'+str(i+1)+'_3')
        print(net)
        out_levels += [net]
        if i == 0:
          top_layer = net
      ...
      net = layers.convolution2d(net, 1024, 1, scope='conv7_3')
