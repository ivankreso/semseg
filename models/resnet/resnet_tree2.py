...
def _build(image, is_training):
  ...
  pool_sizes = [2, 4, 8]
  multiscale = []
  multiscale += [layer(l, 'group3', 512, defs[3], 1, rate=2)]
  l = tf.nn.relu(l)
  for i, s in enumerate(pool_sizes):
    net = layers.avg_pool2d(l, kernel_size=[s, s], stride=[s, s], padding='SAME')
    with tf.variable_scope('', reuse=True):
      multiscale += [layer(net, 'group3', 512, defs[3], 1)]
  print(multiscale)
  ...
  with tf.variable_scope('head'):
    with arg_scope([layers.convolution2d],
      ...
      _, up_h, up_w, _ = multiscale[0].get_shape().as_list()
      for i, l in enumerate(multiscale):
...
def _buildold(image, is_training):
  #def BatchNorm(x, use_local_stat=None, decay=0.9, epsilon=1e-5):
  weight_decay = 1e-4
  global bn_params
  bn_params = {
